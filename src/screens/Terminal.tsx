...
function TerminalContent() {
  ...
  // Scroll to bottom when exiting vim
  useEffect(() => {
    if (wasVimActiveRef.current && !isVimActive) {
      // Vim just closed - scroll terminal to bottom
    ...
    wasVimActiveRef.current = isVimActive
  }, [isVimActive])

  return (
    <>
      {/* Vim Editor Overlay - shown when vim is active */}
      {isVimActive && (
      ...
      {/* Terminal - always mounted but hidden when vim is active */}
      <div className={isVimActive ? 'hidden' : ''}>
        <div className="flex flex-col h-[calc(100vh-40px)] bg-black">
          {/* Terminal Header */}
